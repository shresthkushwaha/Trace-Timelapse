...
const TimerSetupScreen = ({
    ...
    return (
        <div className="w-full h-full relative bg-white overflow-hidden flex flex-col font-sans select-none">
            {/* Top Area: Interval & Duration Split */}
            <div className="flex-1 flex flex-row min-h-0">
                ...
                {/* Middle: Session Duration (Dark) */}
                <div className="w-[30%] bg-[#1E1E1E] relative flex flex-col items-center justify-center text-[#EAE9E5] shrink-0">
                    <div className="flex flex-col items-center justify-center h-full relative w-full pt-10">
                        {/* Up Arrow */}
                        <motion.button
                        ...
                        <div className="relative flex items-center justify-center">
                            <div className="flex flex-col items-center relative h-[120px] justify-center overflow-hidden w-[200px]">
                                ...
                                <AnimatePresence mode="popLayout" initial={false}>
                                    <motion.span
                                        ...
                                        exit={{ y: -50, opacity: 0 }}
                                        transition={{ type: "spring", stiffness: 300, damping: 30 }}
                                        className="text-[120px] font-normal text-[#EAE9E5] leading-[0.9] font-cabinet tracking-tight block text-center w-full"
                                    >
                                        {duration}
                                    </motion.span>
                                </AnimatePresence>
                            ...
                            {/* Unit Selector (Hrs / Min) */}
                            <div className="flex flex-col items-start absolute -right-4 top-1/2 -translate-y-1/2 gap-1 translate-x-full">
                                <motion.button
                                    onClick={() => setDurationUnit('hours')}
                                    className={`text-[24px] font-cabinet font-normal leading-none transition-colors ${durationUnit === 'hours' ? 'text-[#EAE9E5]' : 'text-[#EAE9E5]/20 hover:text-[#EAE9E5]/50'}`}
                                    whileHover={{ scale: 1.05, x: 2 }}
                                >
                                    Hrs
                                </motion.button>
                                <motion.button
                                    ...
                                    className={`text-[24px] font-cabinet font-normal leading-none transition-colors ${durationUnit === 'min' ? 'text-[#EAE9E5]' : 'text-[#EAE9E5]/20 hover:text-[#EAE9E5]/50'}`}
                                    whileHover={{ scale: 1.05, x: 2 }}
                                >
                                    Min
                                </motion.button>
                    ...
                    <span className="text-[16px] font-normal text-[#EAE9E5]/80 absolute bottom-8 left-8 font-cabinet">
                        Session Duration
                    </span>
                </div>
                ...
                <div className="flex-1 bg-white relative flex flex-col min-w-0">
                    ...
                    {/* Estimates Row */}
                    <div className="flex justify-between items-end px-12 pb-12 w-full">
                        <div className="flex flex-col items-center w-1/3">
                            <span className="text-[28px] font-normal text-[#1E1E1E] font-cabinet mb-1">
                                {(() => {
                                    // Calculate Duration in Seconds
                                    let durSec = duration * 60; // default min
                                    if (durationUnit === 'hours') durSec = duration * 3600;

                                    // Calculate Interval in Seconds
                                    let intSec = interval; // default sec
                                    if (intervalUnit === 'min') intSec = interval * 60;

                                    const frames = Math.floor(durSec / intSec);
                        ...
                        <div className="flex flex-col items-center w-1/3">
                            <span className="text-[28px] font-normal text-[#1E1E1E] font-cabinet mb-1">
                                {(() => {
                                    ...
                                    const val = Math.floor(durSec / intSec);
                        ...
                        <div className="flex flex-col items-center w-1/3">
                            <span className="text-[28px] font-normal text-[#1E1E1E] font-cabinet mb-1">
                                {(() => {
                                    ...
                                    const frames = Math.floor(durSec / intSec);
                                    // Approx 0.25 MB per frame
                                    const val = (frames * 0.25).toFixed(2);
                                    return isFinite(frames) ? `${val} MB` : "-- MB";
                                })()}
                            </span>
